...
{
public:
  explicit ScopedFileLock(const int fd)
  : fd_(fd)
  {
    if (fd_ < 0) {
      return;
...
int Nv12ToJpegEncoder::ensure_tacv_lock_fd()
{
  if (tacv_lock_fd_ < 0) {
    tacv_lock_fd_ = open(
      "/tmp/topsbot_webviz_tacv_jpeg.lock",
      O_CREAT | O_CLOEXEC | O_RDWR, 0666);
  }
  return tacv_lock_fd_;
}
  ...
  std::vector<uint8_t> & jpeg_out)
{
  ...
  // The board exposes one JPEG HW encoder. Serialize ta-cv calls across both
  // gateway threads and separate multi_instance processes.
  ScopedFileLock tacv_lock(ensure_tacv_lock_fd());
  if (!tacv_lock.locked()) {
    return false;
  }
...
#if defined(TOPSBOT_WEBVIZ_HAS_TACV)
  if (enable_tacv_ && encode_tacv(nv12, width, height, quality, jpeg_out)) {
    return true;
  }
#endif
