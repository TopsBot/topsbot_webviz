...
| `ros-humble-tb-img-msgs` / `ros-humble-tb-det-msgs` | 订阅零拷贝图像与检测框消息 |
...
| 项 | 默认值 |
|----|--------|
| 场景 yaml | `tbmem_jpeg_zc.yaml`（订阅 `/tbmem_jpeg`） |
...
适用：已启动 YOLO 并发布 `/detections`。Launch：同上 `webviz.launch.py`，使用 `tbmem_img_yolo.yaml`。
...
| launch 参数 | 作用 |
|-------------|------|
| `params_file` | `tbmem_img_yolo.yaml` 已默认开启 `detection_enabled: true` |
...
浏览器 `http://<板卡IP>:8000/`，webviz 使用内置 `tbmem_img_yolo.yaml`。

## 四路预览模式

适用：多路 USB 相机同屏。Launch：`webviz_4ch.launch.py`。
...
```bash
# MES20 — multi_instance 部署（每路独立节点）
ros2 launch topsbot_webviz webviz_4ch.launch.py params_file:=4ch_multi.yaml
```

四路各通道的 `input_topic`、`input_msg` 等在 `4ch_gateway.yaml` / `4ch_multi.yaml` 中配置。
...
| Web 传输 | `ws_base_port`、`gateway_ws_port`、`jpeg_quality`、`queue_max` | WebSocket 端口与 JPEG 编码 |
...
|---------------|-------------|----------|
| `tbmem_jpeg_zc.yaml` | `tb_jpeg` | `mjpeg_640x480_zc.yaml`（**launch 默认**） |
...
| `tbmem_img_yolo.yaml` | `tb_img` + 检测 | 相机 + `topsbot_yolo_det` |
...
| Launch | 用途 |
|--------|------|
| `webviz.launch.py` | 单路预览（及检测框叠加） |
| `webviz_4ch.launch.py` | 四路预览 |
